...
def make_bags_dirichlet_by_class_noreplace(
    data,
    labels,
    bagsize,
    num_classes,
    ...
    """
    按类别生成 LLP bags（严格全局无放回，不重复样本）：
      1) 先 shuffle + 截断到 data_length = (N//bagsize)*bagsize
      2) 建每个类别的 pool（无放回“牌堆”）
      3) 先抽一个 Dirichlet 风味的 counts 矩阵（每行和=bagsize）
      4) 再把 counts 的列和配平到每类真实供给（保证截断批次 100% 用完）
      5) 按 counts 从每类 pool 里切片取样（无放回）
    返回值接口与你原 make_bags_dirichlet 保持一致。
    """
    ...
    if labels.ndim != 1:
        raise ValueError(
            "make_bags_dirichlet_by_class_noreplace 仅支持单标签分类（labels 为一维类别ID）。"
            "你现在的 labels 可能是 multi-hot，多标签数据不能按类别池子严格无放回分配。"
        )
    ...
    # 1) shuffle + truncate（只用这一批，后面保证全用完）
    perm_all = np.arange(dataset_length)
    rng.shuffle(perm_all)
    ...
    if class_sizes.sum() != data_length:
        # 说明 labels 里存在不在 [0, num_classes-1] 的值，或者 num_classes 传错
        raise ValueError(
            f"class_sizes.sum()={class_sizes.sum()} != data_length={data_length}. "
            f"请检查 num_classes={num_classes} 与 labels 的取值范围是否一致。"
        )
    ...
    # 3) sample counts matrix with Dirichlet flavour (row-sum = m)
    #    Dirichlet 参数：alpha0 * ones（你说的“参数=1”就是 alpha0=1.0）
    ...
    # 4) balance column sums to exactly match class_sizes (so we use ALL truncated samples)
    ...
    C0 = _sample_counts_matrix(B, m, Cn, alpha0, rng)
    C = _balance_columns_to_targets(C0, class_sizes, rng)

    # 5) allocate indices without replacement according to C
    ...
    for j in range(num_bags):
        ...
        for c in range(num_classes):
            ...
            s = ptr[c]
            e = s + need
            chosen = pools[c][s:e]  # 一定够，因为列和已经匹配 class_sizes
            ptr[c] = e
...
def load_data_train(args,num_classes, dataset='CIFAR10', dspth='./data', bagsize=16):
    ...
    data_u, label_prob, labels_real, labels_idx, dataset_length, indices_u, input_dim = \
        make_bags_dirichlet_by_class_noreplace(
            data, labels, bagsize, num_classes, dataset, input_dim,
            alpha0=args.pi,  # 你要“狄利克雷参数=1”就用 1.0
            seed=0
        )
    return data_u, label_prob, labels_real, labels_idx, dataset_length, indices_u, input_dim
