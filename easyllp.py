...
def train_one_epoch(epoch,
    ...
    for it in range(len(dltrain_u)):
        ...
        logits_u_w = logits_u_w.squeeze(-1)  # remove last dim → shape [*, C]

        # Compute probabilities
        probs = F.softmax(logits_u_w, dim=-1)  # shape [*, C]

        # Compute (1 - x)^2
        result = (1 - probs).pow(2)  # shape [*, C]

        # Assume instance_weight has the same shape [*, C]
        weighted_result = result * instance_weight
        loss_easy = nn.CrossEntropyLoss()(logits_u_w,instance_weight).mean()
        #loss_easy=weighted_result.mean()
        loss=loss_easy
        b=0.3
        #loss = (loss - b).abs() + b
        optim.zero_grad()
...
def main():
    ...
    parser.add_argument('--exp-dir', default='easy', type=str, help='experiment id')
    parser.add_argument(
