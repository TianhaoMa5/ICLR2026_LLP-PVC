...
def set_model(args,input_dim):
    if args.dataset in ['CIFAR10', 'SVHN', 'CIFAR100', 'miniImageNet','SVHN']:
...
def llp_loss_batch(args,labels_proportion, y, reduce: str = "sum", eps: float = 1e-7):
    """
    labels_proportion: [B, C]  # 每个 bag 的真实比例
    y: [B, C]                  # 每个 bag 的预测比例（如 bag 内平均 softmax 概率）
    ...
    # MSE 部分：调用接口（先不做 reduction，保留 [B, C]，再按 dim=1 做均值）
    mse_loss = F.mse_loss(y, x)  # [B]

    # 合并 loss（你原版是直接返回 cross_entropy，这里可灵活处理）
    loss = mse_loss # 如果你只要 CE，就用这个；要加 MSE 就改成 cross_entropy + mse_loss
...
def train_one_epoch(epoch,
    ...
    for it in range(len(dltrain_u)):
        ...
        loss_prop=llp_loss_batch(args,proportion,bag_preds,reduce="mean")
        x=1.2
        kl_divergence = x
        ...
        probs = torch.softmax(logits_u_w, dim=1).detach()
        ...
        x = loss_prop * bagsize
        loss = (loss_prop * bagsize  - ( bagsize-1) * loss_debais)
        loss.backward()
        optim.step()
        lr_schdlr.step()
...
def main():
    ...
    parser.add_argument('--exp-dir', default='Debais', type=str, help='experiment id')
